...
#### 3.1. Триаж некоторых уязвимостей
...
Сетевая доступность: из любой сети, где доступен веб-сервер (интернет/интранет).
Потенциальный ущерб для бизнеса: критический — полный контроль над сервером может привести к утечке всех данных, установке ransomware, финансовым потерям в миллионы рублей, репутационному краху и штрафам по ФЗ-152 (персональные данные).
Приоритет: самый высокий (1) — немедленное отключение debug-режима и удаление маршрута консоли.
...
Потенциальный ущерб для бизнеса: критический — возможна установка вредоносного ПО, шифрование данных (ransomware), кража конфиденциальной информации, прямые финансовые потери и регуляторные санкции.
Приоритет: самый высокий (1) — переход на shell=False и валидацию ввода.
...
Оценка CVSS: 9.8 (Critical).
Сетевая доступность: из любой сети с доступом к функции десериализации (интернет/интранет).
Потенциальный ущерб для бизнеса: высокий — компрометация сервера, утечка данных, финансовые и репутационные потери, возможные судебные иски за нарушение защиты данных.
Приоритет: высокий (2) — замена pickle на безопасные форматы (json).


SQL-инъекция
Обнаружено: OWASP ZAP.
Верификация: подтверждено — обход аутентификации и извлечение данных из БД.
Воздействие: утечка/модификация данных в базе SQLite.
Оценка CVSS: 9.1 (Critical).
Сетевая доступность: из любой сети с доступом к формам ввода (интернет/интранет).
Потенциальный ущерб для бизнеса: высокий — утечка персональных данных клиентов, штрафы по ФЗ-152 и GDPR-подобным нормам, репутационные потери, возможные судебные претензии.
Приоритет: высокий (3) — переход на параметризованные запросы.
...
Потенциальный ущерб для бизнеса: критический — полный контроль над сервером, аналогичный RCE, с финансовыми, репутационными и регуляторными последствиями.
Приоритет: высокий (4) — экранирование ввода в шаблонах.
...
Сетевая доступность: только при доступе из интернета (clickjacking требует внешнего сайта).
Потенциальный ущерб для бизнеса: средний — возможные атаки на пользователей, репутационные потери, косвенные финансовые риски.
Приоритет: средний — добавить заголовки безопасности.
...
Приоритет: низкий — скрыть заголовки Server.
#### 3.2. Таблица
